...
const run_1 = __importDefault(__nccwpck_require__(792));
const post_run_1 = __importDefault(__nccwpck_require__(570));
const POST_RUN = core.getState("post-run");
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        // serve as the entry-point for both main and post-run invocations
        if (POST_RUN) {
            (0, post_run_1.default)();
        }
        else {
            (0, run_1.default)();
        }
...
function isValidLogOutputIf(str) {
    return /true|false|failure|exit-early|timeout|success/.test(str || "");
}
function normalizeInputs(inputs) {
    let { run, tail: rawTail, logOutput: rawLogOutput, logOutputResume: rawLogOutputResume, logOutputIf, workingDirectory, } = inputs;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const core = __importStar(__nccwpck_require__(186));
const input_1 = __importStar(__nccwpck_require__(139)), inputs = input_1;
const fs = __importStar(__nccwpck_require__(147));
const path_1 = __importDefault(__nccwpck_require__(17));
const { logOutput, logOutputResume, logOutputIf, workingDirectory } = input_1.default;
...
const stdoutPath = path_1.default.join(cwd, `${pid}.out`);
const stderrPath = path_1.default.join(cwd, `${pid}.err`);
const shouldLog = logOutputIf === "true" ||
    logOutputIf === reason ||
    (logOutputIf === "failure" &&
...
function streamLogs() {
    return __awaiter(this, void 0, void 0, function* () {
        if (logOutput.stdout) {
            ...
            yield core.group(`${logOutputResume.stdout ? "Truncated " : ""}Output:`, () => __awaiter(this, void 0, void 0, function* () {
                if (truncated)
                    console.log(`Truncated ${start} bytes of tailed stdout output`);
                yield streamLog(stdoutPath, start);
            }));
        }
        if (logOutput.stderr) {
            const start = logOutputResume.stderr ? stderr : 0;
            const truncated = start > 0;
            yield core.group(`${logOutputResume.stderr ? "Truncated " : ""}Error Output:`, () => __awaiter(this, void 0, void 0, function* () {
                if (truncated)
                    console.log(`Truncated ${start} bytes of tailed stderr output`);
                yield streamLog(stderrPath, start);
            }));
        }
    });
}
function postRunMain() {
    return __awaiter(this, void 0, void 0, function* () {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const core = __importStar(__nccwpck_require__(186));
const tail_1 = __nccwpck_require__(824);
const path_1 = __importDefault(__nccwpck_require__(17));
...
function exitHandler(error, reason) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (stdout && stdout.pos)
            core.saveState("stdout", stdout.pos);
        if (stderr && stderr.pos)
            core.saveState("stderr", stderr.pos);
        if (error) {
            core.error(error);
...
function runCommand(run) {
    ...
    if (pipeStdout)
        cmd += ` > ${process.pid}.out`;
    if (pipeStderr)
        cmd += ` 2> ${process.pid}.err`;
    const shell = (0, child_process_1.spawn)("bash", ["--noprofile", "--norc", "-eo", "pipefail", "-c", cmd], Object.assign({ detached: true, stdio: "ignore" }, (workingDirectory ? { cwd: workingDirectory } : {})));
...
function TailWrapper(filename, shouldTail, output) {
    if (!shouldTail)
        return undefined;
    try {
        const tail = new tail_1.Tail(filename, { flushAtEOF: true });
        tail.on("line", output);
